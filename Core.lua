...
-- Function to filter player buffs via a blacklist
local function SetupBuffFilter()
    if not Perskan.db.profile.enableBuffFilter then
        return
    end

    local showFiltered = false
    local hiddenCount = 0
    local prevHiddenCount = 0
    local toggleButton = nil

    -- Create the toggle button next to the buff frame
    local function CreateToggleButton()
        ...
        -- Position next to the collapse button if it exists, otherwise near BuffFrame
        if BuffFrame.CollapseAndExpandButton then
            if BuffFrame.AuraContainer and BuffFrame.AuraContainer.addIconsToRight then
                btn:SetPoint("RIGHT", BuffFrame.CollapseAndExpandButton, "LEFT", -2, 0)
            else
                btn:SetPoint("LEFT", BuffFrame.CollapseAndExpandButton, "RIGHT", 2, 0)
            end
        else
            btn:SetPoint("TOPLEFT", BuffFrame, "TOPRIGHT", 2, -6)
        end
        ...
        countText:SetPoint("TOPRIGHT", btn, "TOPRIGHT", 4, 4)
        countText:SetFont(countText:GetFont(), 11, "THICKOUTLINE")
        ...
        btn:SetScript("OnEnter", function(self)
            ...
            GameTooltip:AddLine("Perskan Buff Filter")
            GameTooltip:AddLine("Click to toggle hidden buffs", 1, 1, 1, true)
            GameTooltip:AddLine("Shift+Alt+RightClick a buff to blacklist/unblacklist it", 0.7, 0.7, 0.7, true)
            GameTooltip:Show()
        end)
    ...
    -- Build a set of icon textures for blacklisted spells (for combat fallback)
    local blacklistedTextures = {}
    local function RebuildTextureCache()
        wipe(blacklistedTextures)
        for spellId, _ in pairs(Perskan.db.profile.buffBlacklist) do
            local info = C_Spell.GetSpellInfo(spellId)
            if info and info.iconID then
                blacklistedTextures[info.iconID] = spellId
            end
        end
    end
    RebuildTextureCache()

    -- Post-hook: filter and reposition after Blizzard finishes
    hooksecurefunc(BuffFrame, "UpdateAuraButtons", function(self)
        if not Perskan.db.profile.enableBuffFilter then return end
        ...
        if not auraInfo or not auraFrames then return end

        local container = self.AuraContainer
        local iconStride = container and container.iconStride or 10
        local iconPadding = container and container.iconPadding or 2
        local addIconsToRight = container and container.addIconsToRight
        local addIconsToTop = container and container.addIconsToTop
        local iconScale = container and container.iconScale or 1

        -- Read actual icon size from the first visible aura frame
        local iconSize = 30
        for _, f in ipairs(auraFrames) do
            if f and f:IsShown() and not f.isAuraAnchor then
                iconSize = f:GetWidth()
                break
            end
        end

        -- Tag frames with spellId, classify as visible or hidden
        local visibleFrames = {}
        local hiddenFrames = {}
        for i, info in ipairs(auraInfo) do
            ...
            if frame and not frame.isAuraAnchor then
                ...
                if info.auraType == "TempEnchant" then
                    local hasMainHand, _, _, mainHandId = GetWeaponEnchantInfo()
                    if hasMainHand then
                        spellId = mainHandId
                    end
                else
                    local ok, _, _, _, _, _, _, _, _, _, sid = pcall(function()
                        local auraData = C_UnitAuras.GetAuraDataByIndex("player", info.index, "HELPFUL")
                        if auraData then
                ...
                -- Fallback: match by icon texture (texture can be secret in combat)
                if not spellId and frame.Icon then
                    pcall(function()
                        local texture = frame.Icon:GetTexture()
                        if texture and blacklistedTextures[texture] then
                            spellId = blacklistedTextures[texture]
                        end
                    end)
                end

                if spellId then
                    frame._perskanSpellId = spellId
                end

                -- Check blacklist (cached spellId could theoretically be secret)
                local isBlacklisted = false
                pcall(function()
                    local checkId = frame._perskanSpellId
                    if checkId and blacklist[checkId] then
                        isBlacklisted = true
                    end
                end)

                -- Attach Shift+Alt+RightClick hook once per frame
                if not frame._perskanClickHooked then
                    frame._perskanClickHooked = true
                    frame:HookScript("OnMouseUp", function(self, button)
                        if button == "RightButton" and IsShiftKeyDown() and IsAltKeyDown() then
                            ...
                            if id then
                                ...
                                if bl[id] then
                                    bl[id] = nil
                                    print("|cff00ccffPerskan|r: Removed from blacklist: " .. (C_Spell.GetSpellName(id) or id))
                                else
                                    bl[id] = C_Spell.GetSpellName(id) or ("Spell " .. id)
                                    print("|cff00ccffPerskan|r: Added to blacklist: " .. bl[id])
                                end
                                RebuildTextureCache()
                                if not InCombatLockdown() then
                                    BuffFrame:UpdateAuraButtons()
        ...
        -- Reposition all visible frames in a grid when filtering is active or was just cleared
        if hiddenCount > 0 or prevHiddenCount > 0 then
            prevHiddenCount = hiddenCount
            local spacingX = iconPadding
            local spacingY = iconPadding + 8

            for idx, frame in ipairs(visibleFrames) do
                local row = math.ceil(idx / iconStride)
                local col = ((idx - 1) % iconStride)

                local xOff = col * (iconSize + spacingX)
                local yOff = (row - 1) * (iconSize + spacingY)

                if not addIconsToRight then
                    xOff = -xOff
                end
                if not addIconsToTop then
                    yOff = -yOff
                end
                ...
                if addIconsToRight then
                    if addIconsToTop then
                        frame:SetPoint("BOTTOMLEFT", container, "BOTTOMLEFT", xOff, yOff)
                    else
                        frame:SetPoint("TOPLEFT", container, "TOPLEFT", xOff, yOff)
                    end
                else
                    if addIconsToTop then
                        frame:SetPoint("BOTTOMRIGHT", container, "BOTTOMRIGHT", xOff, yOff)
                    else
        ...
        -- Show hidden frames stacked downward from toggle button if toggled
        if showFiltered and hiddenCount > 0 then
            local spacing = 2
            for idx, frame in ipairs(hiddenFrames) do
                frame:ClearAllPoints()
        ...
        if hiddenCount > 0 then
            btn.countText:SetText(hiddenCount)
            btn:Show()
        else
            btn:Hide()
            showFiltered = false
    ...
    Perskan.RebuildBuffFilterCache = RebuildTextureCache
    Perskan.RefreshBuffFilter = function()
        RebuildTextureCache()
