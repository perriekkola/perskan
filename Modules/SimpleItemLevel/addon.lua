-- [Perskan] Vendored from Simple Item Level v67 (Kemayo, BSD). Four marked edits:
-- this namespace/name pinning, the saved-variable name, the ADDON_LOADED test and
-- the optional SetupConfig call. Everything else is upstream.
local myname, ns = "SimpleItemLevel", PerskanSimpleItemLevel
local myfullname = "Simple Item Level"
local db
...
function ns:ADDON_LOADED(event, addon)
    ...
    if addon == ns.HOST_ADDON then -- [Perskan] fires with the host addon, not a standalone one
        -- [Perskan] Kept in Simple Item Level's own saved variable, so an existing
        -- install's settings carry over and upstream's defaults still apply through
        -- the metatable below.
        _G["SimpleItemLevelDB"] = setmetatable(_G["SimpleItemLevelDB"] or {}, {
            __index = ns.defaults,
        })
        db = _G["SimpleItemLevelDB"]
        ns.db = db

        if ns.SetupConfig then -- [Perskan] config.lua isn't vendored; Perskan's window drives these
            ns:SetupConfig()
        end
