...
function BindPadFrame_Toggle()
    if not BindPadCore.IsEnabledByPerskan() then
...
function BindPadCore.InitBindPadOnce(event)
    -- [Perskan] Don't claim any keybindings while the feature is switched off. Left
    -- uninitialised rather than flagged, so switching it on and reloading still works.
    if not BindPadCore.IsEnabledByPerskan() then
        return
    end
